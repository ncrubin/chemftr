...
def cost_thc(n: int, lam: float, dE: float, chi: int, beta: int, M: int, stps: int,
    ...
    Args:
        ...
        chi (int) - equivalent to aleph in the document, the number of bits for the representation
            of the coefficients
    ...
    # The output size is 2*log[M] for the alt values, chi for the keep value, and 2 for the two sign
    # bits, as in Eq. (29).
    m = 2 * nM + 2 + chi
    oh = [0] * 20
    for p in range(20):
        ...
        oh[p] = stps * (1 / (np.sin(3 * arcsin(np.cos(v * 2 * np.pi / np.power(2,p+1)) * \
            np.sqrt(d) / np.power(2,nM)))**2) - 1) + 4 * (p + 1)

    # Set it to be the number of bits that minimises the cost, usually 7. JJG note: this will be 6
    # on 0-index like Python, so need to add the one back in
    br = np.argmin(oh) + 1

    # This is the costing for preparing the equal superposition over the input registers from below
    # Eq. (27).
    cp1 = 2 * ( 10 * nM + 2 * br - 9)

    # This is the cost of computing the contiguous register and inverting it. This is with a
    # sophisticated scheme adding together triplets of bits. This is the cost of step 2 in the list
    # on pages 15 and 16, with a factor of 2 to account for its inverse.
    cp2 = 2 * (nM **2 + nM - 1)

    # This is the cost of the QROM for the state preparation in step 3 and its inverse.
    # JJG note: arg_min is first value, min is second value
    cp3 = QR(d,m)[1] + QI(d)[1]
    ...
    # Then there is a cost of nM + 1 for swapping the mu and nu registers in step 6, where the + 1
    # is because we need to control on two registers. There is the same cost for the inverse.
    cp6 = 2 * nM + 2

    # This is the total cost in Eq. (32).
    CPCP = cp1 + cp2 + cp3 + cp4 + cp5 + cp6

    # This is the cost of swapping based on the spin register. It is steps 1 and 7 from the list of
    # steps on pages 16 and 17.
    cs1 = 2 * n

    # The QROM for the rotation angles the first time in step 2. and the second part is the cost for
    # inverting them with advanced QROM (step 6).
    cs2a = M + n/2 - 2

    # The QROM for the rotation angles the second time. Here the cost M - 2 is for generating the
    # angles the second time (again step 2), and QI[M] is for inverting the QROM (step 6).
    cs2b = M - 2

    # The cost of the rotations in steps 3 and 5, which must be done 4 times.
    cs3 = 4 * n * (beta - 2)

    # Cost for making the Z doubly controlled in step 4, and a controlled swap of the spin qubits.
    # JJG note: is this really just "2"? Does this need a variable?
    cs4 = 2

    # JJG note: want the argument, so that is first element
    ...
    # The cost for inverting the rotation angles the second time in step 6. JJG note: use value,
    # so second output argument
    cs6b = QI(M)[1]
    ...
    # This is for the equal superposition state to perform the inequal. test with the keep register.
    ac4 = chi
    ...
    # JJG note we want arg_min so first argument
    ...
    # These are the ancillas needed for adding into the phase gradient state in part 14.
    ac14 = beta - 2

    # These are the temporary ancillas in between erasing the first QROM ancillas and inverting
    # that QROM. The + m is the for output of the first QROM.
    acc = ac13 + ac14 + m
