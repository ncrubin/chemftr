...
def compute_cost(n: int, lam: float, dE: float, L: int, chi: int, stps: int,
    ...
    # Cost of preparing the equal superposition state on the first register in step 1 (a).
    ...
    # The inequality test of cost chi in step 1(c) and the controlled swap with cost nL + 1 in
    # step 1(d) (and their inversions).
    cost1cd = 2 * (chi + nL + 1)

    oh = [0] * 20
    nprime = int(n**2//8 + n//4)
    for p in range(20):
        v = np.round(np.power(2,p+1) / (2*np.pi) * arccos(np.power(2, 2*nN) / np.sqrt(nprime) / 2))
        oh[p] = np.real(20000 * (1 / (np.sin(3 * arcsin(np.cos(v * 2 * np.pi / np.power(2, p+1)) * \
            np.sqrt(nprime) / np.power(2,2*nN)))**2) - 1) + 4 * (p + 1))

    # Bits of precision for rotation for preparing the next equal superposition states
    br = int(np.argmin(oh) + 1)
    ...
    # The cost of the inequality test and controlled swap for the quantum alias sampling in
    # steps 2 (d) and (e).
    cost2de = 4 * (chi + 2 * nN)

    # Swapping the p & q registers for symmetry in step 3.  This needs to be done and inverted
    # twice, hence the factor of 4.
    cost3 = 4 * nN

    # The SELECT operation in step 4, which needs to be done twice.
    cost4 = 4 * n - 8

    # The reflection used on the second register in the middle, with the cost in step 6.
    cost6 = 2 * nN + chi + 3

    # Step 7 involves performing steps 2 to 5 again, which are accounted for above, but there is
    # one extra Toffoli for checking that l <> 0.
    cost7 = 1

    # The total reflection for the quantum walk in step 9.
    cost9 = nL + 2*nN + 2*chi + 2

    # The two Toffolis for the control for phase estimation and iteration, given in step 10.
    cost10 = 2
    ...
    cost = cost1a + cost1b + cost1cd + cost2a + cost2b + cost2c + cost2de + cost3 + cost4 + cost6 +\
           cost7 + cost9 + cost10
