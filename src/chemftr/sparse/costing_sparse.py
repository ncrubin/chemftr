...
def cost_sparse(n: int, lam: float, d: int, dE: float, chi: int, stps: int) -> Tuple[int, int, int]:
    """ Determine fault-tolerant costs using sparse decomposition in quantum chemistry
    ...
    # JJG note: I think there is a bug in the mathematica notebook. It does not check if 2 is a
    # factor first, which it should, cf. the similar function in costingdf.nb
    # Below is correct using the power_two() function, to give power of 2 that is a factor of d.
    eta = power_two(d)
    ...
    # FIXME: is nM integer or not? I think it is integer, but in Li case, e.g., it is 19/2
    ...
    for p in range(2,22):
        ...
        v = np.round(np.power(2,p+1) / (2 * np.pi) * arccos(np.power(2,nM) /\
            np.sqrt(d/2**eta)/2))
        oh[p-2] = np.real(stps * (1 / (np.sin(3 * arcsin(np.cos(v * 2 * np.pi / np.power(2,p+1)) * \
    ...
    # Equation (A17)
    cost = np.ceil(d/k1) + m * (k1 -1) + QI(d)[1] + 4 * n + 8 * nN + 2 * chi + 7 *\
        np.ceil(np.log2(d)) - 6 * eta + 4 * br - 19
    ...
    # The qubit used for flagging the success of the equal superposition state preparation and the
